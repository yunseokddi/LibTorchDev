...
int main()
{
	...
	std::string img_path = "./imgs/dog.jpg";
	...
	cv::resize(img, img, cv::Size(640, 384));
	...
	if (dets.size() > 0)
	{
		...
		{
			float left = dets[0][i][0].item().toFloat() * img.cols / 640;
			float top = dets[0][i][1].item().toFloat() * img.rows / 384;
			float right = dets[0][i][2].item().toFloat() * img.cols / 640;
			float bottom = dets[0][i][3].item().toFloat() * img.rows / 384;
